...
def notify_user_registered(user):
    """
    Sent immediately after any new user signs up.
    Students can start right away; supervisors/admins are told to wait for approval.
    """
    role_display = user.get_role_display()

    if user.role == 'student':
        body = f"""
        <h3>Welcome to ILES, {user.username}!</h3>
        <p>Your <b>{role_display}</b> account has been created successfully.</p>
        <p>You can log in now and start managing your internship.</p>
        <p><a href="http://your-frontend-url/login">Go to the ILES portal</a></p>
        <p style="color:#888; font-size:12px;">This is an automated message from the ILES Portal.</p>
        """
    else:
        body = f"""
        ...
        <p>Your <b>{role_display}</b> account has been created successfully.</p>
        <p>Your account is currently <b>pending admin approval</b>. You will receive
        another email as soon as an administrator activates your account.</p>
        <p style="color:#888; font-size:12px;">This is an automated message from the ILES Portal.</p>
        """

    send_email(
        subject="Welcome to the ILES Portal",
        recipient_email=user.email,
        message_html=body,
    )


# NOTIFICATION WHEN AN ISSUE IS RESOLVED
def notify_student_issue_resolved(issue):
    """
    Sent to the student when a supervisor or admin marks their issue as Resolved.
    """
    try:
        student_name = issue.student.student_profile.student_name
    except Exception:
    ...
    send_email(
        subject=f"Your Issue Has Been Resolved — {issue.title}",
        recipient_email=issue.student.email,
        message_html=f"""
        <h3>Hello {student_name},</h3>
        <p>Your issue has been marked as <b>Resolved</b>.</p>
        <ul>
            <li><b>Title:</b> {issue.title}</li>
            <li><b>Description:</b> {issue.description}</li>
        </ul>
        {"<p><b>Supervisor feedback:</b> " + issue.supervisor_feedback + "</p>"
          if issue.supervisor_feedback else ""}
        <p>Log in to the ILES portal to view the full details.</p>
        <p style="color:#888; font-size:12px;">This is an automated message from the ILES Portal.</p>
        """,
    )
